import requests
#import config
from teams_and_players.models import Player, Team, TeamsToTransferUpdate
import json
import time
import os
...
def get_players_run():
    ids = TeamsToTransferUpdate.objects.all().first()
    print(ids.team.id)

   
    get_players(ids.team.id)
    print('sleep for 20 seconds')
    time.sleep(20)
        

    print(f'{ids} players added to db')
